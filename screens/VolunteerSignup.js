...
export default function VolunteerSignup() {
  ...
  const submit = async (data) => {
    ...
    try {
      const response = await fetch('http://10.0.2.2:4000/volunteer/signup', {
        method: 'POST',
        headers: {
  ...
  return (
    <View className="bg-gray-200 flex-1 justify-center w-full h-full">
      <View className="w-full flex flex-col justify-center px-10">
        ...
        <Text className="text-black font-bold text-xl mb-2">Volunteer</Text>
        ...
        <Controller
          ...
          render={({ field: { onBlur, onChange, value } }) => (
            <Animated.View entering={FadeInDown.duration(1000)} className="w-[45%] mb-1">
...
<Controller
          ...
          render={({ field: { onBlur, onChange, value } }) => (
            <Animated.View entering={FadeInDown.duration(1000)} className="w-1/2 mb-1">
              <View className="bg-gray-300 p-4 rounded-lg w-full">
                <TextInput
        ...
        <TouchableOpacity className="w-full bg-[#988AC7] p-4 rounded-full" onPress={handleSubmit(submit)}>
