...
export default function DonerSignup() {
  ...
  const submit = async (data) => {
    ...
    try {
      const response = await fetch('http://10.0.2.2:4000/doner/signup',  {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error('Failed to regisater user:', text);
        alert(`Failed to regiaster user: ${text}`);
      }
  ...
  return (
    <View className="bg-gray-200 flex-1 justify-center w-full h-full">
      <View className="w-full flex flex-col justify-center px-10">
        ...
        <Text className="text-black font-bold text-xl mb-2">Food Donor</Text>
        ...
        <TouchableOpacity className="w-full bg-[#659667] p-4 rounded-full" onPress={handleSubmit(submit)}>
